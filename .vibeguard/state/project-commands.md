...
- observed-in-code: Python helper syntax check command is `PYTHONPYCACHEPREFIX="${TMPDIR:-/tmp}/vibeguard-pycache" python3 -m py_compile templates/en/.vibeguard/bin/vibeguard-status.py templates/en/.vibeguard/bin/vibeguard-audit.py templates/zh/.vibeguard/bin/vibeguard-status.py templates/zh/.vibeguard/bin/vibeguard-audit.py .vibeguard/bin/vibeguard-status.py .vibeguard/bin/vibeguard-audit.py`.
...
- verified-by-command: `PYTHONPYCACHEPREFIX="${TMPDIR:-/tmp}/vibeguard-pycache" python3 -m py_compile templates/en/.vibeguard/bin/vibeguard-status.py templates/en/.vibeguard/bin/vibeguard-audit.py templates/zh/.vibeguard/bin/vibeguard-status.py templates/zh/.vibeguard/bin/vibeguard-audit.py .vibeguard/bin/vibeguard-status.py .vibeguard/bin/vibeguard-audit.py` passed.
...
- verified-by-command: `sh install/codex.sh --update --lang zh --version v0.1.4 --yes` passed and reported `state schema: local 1, template 1`.
- verified-by-command: `python3 .vibeguard/bin/vibeguard-status.py` passed with `VibeGuard 状态（status）：ok`.
- verified-by-command: `python3 .vibeguard/bin/vibeguard-audit.py` exited 0 and reported expected `VibeGuard 审计（audit）：attention needed` for governance rule/helper/test changes with state files updated.
