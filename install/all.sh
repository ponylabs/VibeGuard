...
TOOL_NAME="all supported tools"
ENTRY_FILES="AGENTS.md CLAUDE.md .cursor/rules/vibeguard.mdc"
...
usage() {
  if [ "$LANGUAGE" = "zh" ]; then
    cat <<EOF
用法：sh install/all.sh [options]

将 VibeGuard 安装到当前项目，并注入 Codex, Claude Code, and Cursor entries。
...
EOF
  ...
  cat <<EOF
Usage: sh install/all.sh [options]

Installs VibeGuard into the current project and injects Codex, Claude Code, and Cursor entries.
