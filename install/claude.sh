...
usage() {
  if [ "$LANGUAGE" = "zh" ]; then
    cat <<EOF
用法：sh install/claude.sh [options]

将 VibeGuard 安装到当前项目，并注入 CLAUDE.md。
...
EOF
  ...
  cat <<EOF
Usage: sh install/claude.sh [options]

Installs VibeGuard into the current project and injects CLAUDE.md.
